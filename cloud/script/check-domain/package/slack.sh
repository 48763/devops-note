...
if [[ $# -eq 0 ]]; then
  ...
  while getopts ":a:A:b:B:c:C:e:h:i:I:m:N:p:s:t:T:L:k:u:w" opt; do
    case ${opt} in
      a) ATTACHMENT="true" ;;
      A) AUTHOR="${OPTARG}" ;;
      b) AUTHORICON="${OPTARG}" ;;
...
if [[ -n "${MODE}" ]]; then
  ...
  if test -z "${USERNAME}"; then echo "INFO: A USERNAME (-u) was not specified for this POST to the Slack API. Setting a default username..."; USERNAME="${HOSTNAME}"; fi
fi
...
function SEND() {
  ...
  if [[ ${ATTACHMENT} = "true" ]]; then
    PAYLOAD="payload={ \
      ...
      \"fields\": [{\"title\": \"Status\",\"value\": \"${PRIORITY}\",\"short\": \"true\"}, {\"title\": \"Host\",\"value\": \"${HOSTNAME}\",\"short\": \"true\"}, {\"title\": \"Environment\",\"value\": \"${ENV}\",\"short\": \"true\"} ], \
      \"image_url\": \"${IMAGE}\", \
      \"footer\": \"${FOOTER}\", \
  ...
  #echo curl -s -S -X POST --data-urlencode "${PAYLOAD}" "${WEBHOOK}";
